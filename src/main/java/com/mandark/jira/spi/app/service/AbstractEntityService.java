...
/**
 * Abstract implementation of {@link EntityService} for Entities
 * 
 * @param <E> Type Parameter for the Entity
 * @param <ED> DTO Type of the Entity
 */
public abstract class AbstractEntityService<K, E extends IEntity<K>, ED extends EntityDTO<K, E>> //
    ...
    protected AbstractEntityService(IDao<K> dao) {
        super();
    ...
    protected <IE extends IEntity<K>> IE readEntity(final Class<IE> entityCls, final K id,
        ...
        if (errorIfNotFound && Objects.isNull(entityObj)) {
            final String entityName = entityCls.getClass().getSimpleName();
            final String errMsg = String.format("#readEntity :: No Object found with id : %s - %s", entityName, id);
            final String errMsgUser = String.format("Oops! %s resource you're looking for is not found.", entityName);
    ...
    @Override
    public int count(SearchQuery<? extends E> searchQuery) {
        // Sanity checks
    ...
    @Override
    public List<ED> search(SearchQuery<? extends E> searchQuery, int pageNo, int pageSize) {
        // Sanity checks
        if (Objects.isNull(searchQuery)) {
